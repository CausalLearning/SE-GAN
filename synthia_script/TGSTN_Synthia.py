...
def get_arguments():
    ...
    parser.add_argument("--restore_from", type=str, default='/home/yonghao.xu/PreTrainedModel/VGG_pretrained_Synthia.pth',
...
def main():
    ...
    snapshot_dir = args.snapshot_dir+'Synthia/semantic_weight_'+str(args.semantic_weight)+'_perceptual_weight_'+str(args.perceptual_weight)+'_lr'+str(args.learning_rate)+'_advlr'+str(args.learning_rate_D)+'_time'+time.strftime('%m%d_%H%M', time.localtime(time.time()))+'/'
    if os.path.exists(snapshot_dir)==False:
        os.makedirs(snapshot_dir)
    f = open(snapshot_dir+'SynthiaTrans_log.txt', 'w')
    ...
    for epoch in range(args.num_epoch):
        ...
        f.write('Save Model\n') 
        print('Save Model')                     
        model_name = 'SynthiaTrans_epoch_'+repr(epoch+1)+'_batch_'+repr(batch_index+1)+'_miou_'+repr(int(np.mean(loss_hist[index_i-9:index_i+1,5])*100))+'.pth'
