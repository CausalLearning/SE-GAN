...
def get_arguments():
    ...
    #dataset
    parser.add_argument("--data_dir_source", type=str, default='/home/yonghao.xu/Data/SegmentationData/Synthia/',
                        help="source dataset path.")
    parser.add_argument("--data_dir_tgstn", type=str, default='./TGSTN/Synthia/',
                        help="aug dataset path.")
    parser.add_argument("--data_list_source", type=str, default='./dataset/synthia_imagelist_train.txt',
    ...
    parser.add_argument("--st_weight", type=float, default=1,
...
def main():
    ...
    snapshot_dir = args.snapshot_dir+'Synthia/SEGAN_adv_weight_'+str(args.adv_weight)+'_time'+time.strftime('%m%d_%H%M', time.localtime(time.time()))+'/'
    ...
    for i in saved_state_dict:
        i_parts = i.split('.')
        if not args.num_classes == 16 or not i_parts[1] == 'layer5':
    ...
    src_loader = data.DataLoader(
                    synthiaDataSet(args.data_dir_source, args.data_list_source, max_iters=int(np.ceil(args.num_steps_stop*args.batch_size/2.)),
    ...
    for batch_index, (src_data, tgt_data) in enumerate(zip(src_loader, tgt_loader)):
        ...
        tgt_t_input = images + torch.randn(images.size()).cuda()
        tgt_s_input = images + torch.randn(images.size()).cuda()

        tgt_s_output_ori1,tgt_s_output_ori2 = student_net(tgt_s_input)
        tgt_t_output_ori1,tgt_t_output_ori2 = teacher_net(tgt_t_input)
        
        tgt_t_output1 = interp_tgt(tgt_t_output_ori1)
        tgt_t_output2 = interp_tgt(tgt_t_output_ori2)
        tgt_s_output1 = interp_tgt(tgt_s_output_ori1)
        tgt_s_output2 = interp_tgt(tgt_s_output_ori2)
        
        tgt_t_predicts1 = F.softmax(tgt_t_output1, dim=1).transpose(1, 2).transpose(2, 3)
        tgt_s_predicts1 = F.softmax(tgt_s_output1, dim=1).transpose(1, 2).transpose(2, 3)

        tgt_s_predicts1 = tgt_s_predicts1.contiguous().view(-1,n_class)
        tgt_t_predicts1 = tgt_t_predicts1.contiguous().view(-1,n_class)

        tgt_t_predicts2 = F.softmax(tgt_t_output2, dim=1).transpose(1, 2).transpose(2, 3)
        tgt_s_predicts2 = F.softmax(tgt_s_output2, dim=1).transpose(1, 2).transpose(2, 3)

        tgt_s_predicts2 = tgt_s_predicts2.contiguous().view(-1,n_class)
        tgt_t_predicts2 = tgt_t_predicts2.contiguous().view(-1,n_class)
        
        aug_loss_value = aug_loss(tgt_s_predicts1, tgt_t_predicts1)*0.1+aug_loss(tgt_s_predicts2, tgt_t_predicts2)
        ...
        pred_target2 = tgt_s_output2.detach()
        ...
        if (batch_index+1) % 500 == 0:                    
            OA_new = test_mIoU16(f,teacher_net, val_loader, batch_index+1,input_size_test,print_per_batches=10)
