...
def main():
    ...
    snapshot_dir = args.snapshot_dir+'Synthia/ResNet_Pretrain_time'+time.strftime('%m%d_%H%M', time.localtime(time.time()))+'/'
    if os.path.exists(snapshot_dir)==False:
        os.makedirs(snapshot_dir)
    f = open(snapshot_dir+'SynthiaSeg_log.txt', 'w')
    ...
    src_loader = data.DataLoader(
                    synthiaDataSet(args.data_dir_source, args.data_list_source, max_iters=args.num_steps_stop,
    ...
    for batch_index, src_data in enumerate(src_loader):
        ...
        if (batch_index+1) % 10000 == 0: 
            ...
            f.write('Save Model\n') 
            print('Save Model\n')                     
            model_name = 'Synthia_batch_'+repr(batch_index+1)+'_miou_'+repr(int(np.mean(loss_hist[batch_index-9:batch_index+1,1])*100))+'.pth'
