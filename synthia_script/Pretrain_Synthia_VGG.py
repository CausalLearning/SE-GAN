...
def main():
    ...
    snapshot_dir = args.snapshot_dir+'Synthia/VGG_Pretrain_time'+time.strftime('%m%d_%H%M', time.localtime(time.time()))+'/'
    ...
    f.write('Save Model\n') 
    print('Save Model')                     
    model_name = 'Synthia_epoch_'+repr(epoch+1)+'_batch_'+repr(batch_index+1)+'_miou_'+repr(int(np.mean(loss_hist[index_i-9:index_i+1,1])*100))+'.pth'
