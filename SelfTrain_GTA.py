...
def main():
    ...
    snapshot_dir = args.snapshot_dir+'GTA/SelfTrain_adv1e3'+'_time'+time.strftime('%m%d_%H%M', time.localtime(time.time()))+'/'
    ...
    for batch_index, tgt_data in enumerate(tgt_loader):
        ...
        if (batch_index+1) % 500 == 0:                    
            OA_new = test_mIoU(f,selftrain_net, val_loader, batch_index+1,input_size_test,print_per_batches=10)
